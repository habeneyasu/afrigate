...
# Maps TradeDocument field → human-readable document label for documents_present
_CERT_FIELDS: dict[str, str] = {
    "certificate_of_origin_number":     "Certificate of Origin",
    "phytosanitary_certificate_number": "Phytosanitary Certificate",
    "health_certificate_number":        "Health Certificate",
    "fumigation_certificate_number":    "Fumigation Certificate",
    "inspection_certificate_number":    "Inspection Certificate",
    "conformity_certificate_number":    "Certificate of Conformity",
    "invoice_number":                   "Commercial Invoice",
    "packing_list_number":              "Packing List",
    "bill_of_lading_number":            "Bill of Lading",
    "airway_bill_number":               "Airway Bill",
    "export_license_number":            "Export License",
    "import_license_number":            "Import License",
    "halal_certificate_number":         "Halal Certificate",
    "kosher_certificate_number":        "Kosher Certificate",
    "organic_certificate_number":       "Organic Certificate",
    "iso_certificate_number":           "ISO Certificate",
    "msds_number":                      "MSDS / SDS",
    "cites_permit_number":              "CITES Permit",
    "gsp_form_number":                  "GSP Form",
    "eur1_number":                      "EUR.1 Certificate",
}
...
_CONTAINER_PATTERN = re.compile(r"\b([A-Z]{3}U\d{7}|[A-Z]{4}\d{6,7})\b")
...
def _to_iso2(country_name: str | None) -> str | None:
    ...
    if HAS_PYCOUNTRY:
        try:
            ...
            return result[0].alpha_2 if result else None
        except LookupError:
            return None
    # Fallback: manual map for common African trade countries
    _FALLBACK: dict[str, str] = {
        "ethiopia": "ET", "kenya": "KE", "nigeria": "NG", "ghana": "GH",
        "south africa": "ZA", "egypt": "EG", "tanzania": "TZ", "uganda": "UG",
        "rwanda": "RW", "senegal": "SN", "ivory coast": "CI", "morocco": "MA",
        "zambia": "ZM", "zimbabwe": "ZW", "cameroon": "CM", "angola": "AO",
        "mozambique": "MZ", "djibouti": "DJ", "sudan": "SD", "ethiopia": "ET",
        "united states": "US", "united kingdom": "GB", "china": "CN",
        "india": "IN", "germany": "DE", "france": "FR", "netherlands": "NL",
        "united arab emirates": "AE", "saudi arabia": "SA",
    }
    return _FALLBACK.get(country_name.lower())
...
def _build_extracted_fields(doc: TradeDocument) -> ExtractedFields:
    """Map a TradeDocument to the ExtractedFields TypedDict."""
    raw = doc.to_dict()

    weight_raw = raw.get("net_weight") or raw.get("gross_weight")
    weight_kg  = _to_float(weight_raw)

    value_raw = (
        raw.get("invoice_value")
        or raw.get("total_value")
        or raw.get("fob_value")
    )
    value_usd = _to_float(value_raw)

    docs_present = [
        label
        for field_key, label in _CERT_FIELDS.items()
        if raw.get(field_key)
    ]

    return ExtractedFields(
        exporter=            raw.get("exporter_name"),
        importer=            raw.get("importer_name"),
        origin_country=      _to_iso2(raw.get("origin_country") or raw.get("country_of_origin")),
        destination_country= _to_iso2(raw.get("destination_country") or raw.get("importer_country")),
        product=             (raw.get("product_name") or "").lower().strip() or None,
        weight_kg=           weight_kg,
        value_usd=           value_usd,
...
def run_doc_intel(
    ...
    accumulated_certs: dict | None = None,
    ...
    Parameters
    ----------
    user_input        : plain text string (typed or pasted)
    file_path         : path to PDF / DOCX / TXT (overrides user_input)
    accumulated_certs : {field_name: value} carried across iterations
    feedback          : evaluator feedback to trigger mock cert injection

    Returns
    -------
    ExtractedFields TypedDict with ISO 3166-1 alpha-2 country codes.
    """
    if file_path:
        doc = extract_from_file(file_path)
    else:
        doc = extract_from_text(user_input)

    # Re-apply certs from previous iterations
    if accumulated_certs:
        for attr, val in accumulated_certs.items():
            if not getattr(doc, attr, None):
                setattr(doc, attr, val)

    # Feedback-driven mock cert injection
    if feedback:
        fields = doc.to_dict()
        mock_map = {
            "Certificate of Origin":      ("certificate_of_origin_number",    "CO-MOCK"),
            "Phytosanitary":              ("phytosanitary_certificate_number", "PHY-MOCK"),
            "Health Certificate":         ("health_certificate_number",        "HC-MOCK"),
            "Certificate of Conformity":  ("conformity_certificate_number",    "COC-MOCK"),
            "Commercial Invoice":         ("invoice_number",                   "INV-MOCK"),
        }
        for keyword, (attr, prefix) in mock_map.items():
            if keyword in feedback and attr not in fields:
                value = f"{prefix}-{id(doc) % 9000 + 1000}"
                setattr(doc, attr, value)
                if accumulated_certs is not None:
                    accumulated_certs[attr] = value
...
if __name__ == "__main__":
    ...
    for i, sample in enumerate(samples, 1):
        print(f"\n{'='*70}")
        print(f"SAMPLE {i} — TradeDocument (full)")
        print(f"{'='*70}")
        doc = extract_from_text(sample)
        print(doc.summary())

        print(f"\n{'='*70}")
        print(f"SAMPLE {i} — ExtractedFields (run_doc_intel)")
        print(f"{'='*70}")
        ef = run_doc_intel(sample)
        for k, v in ef.items():
            print(f"  {k:<25} {v}")
    ...
    if len(sys.argv) > 1:
        ...
        print(f"FILE: {fp}")
        print(f"{'='*70}")
        ef = run_doc_intel("", file_path=str(fp))
        for k, v in ef.items():
            print(f"  {k:<25} {v}")